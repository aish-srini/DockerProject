// to do:
//   [*]  - combine into one doc (serverclient.go) 
//     - figure out what's to go in the "main" function (if/else condition)
...
func main() {
	...
	if *ednamode != "" {
		...
		fmt.Println("Uh no, that is not a command. Initiating self-destruct in 3...2...1... JUST KIDDING!")
	}	 
}

func client() {
	clientConn, err := net.Dial("tcp", "169.229.50.178:5888")
	if err != nil {
...
//      reader := bufio.NewReader(serverConn)
        ...
        for i:= 0; i < numIters; i++ {
                ...
                if err != nil {
                        fmt.Println("Recieve failed", err)
                        os.Exit(1)
                }
                fmt.Printf("(%d) Recieved: %s", i, recvMsg)

                fmt.Printf("(%d) Sending: %s\n", i, sendMsg)
                if _, err := serverConn.Write([]byte("pingpong server")); err != nil {
                        fmt.Println("Send failed:", err)
                        os.Exit(1)
                }
        }
